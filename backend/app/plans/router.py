...
def list_plans(client_id: int, db: Session = Depends(get_db)):
...
def list_objectives(goal_id: int, db: Session = Depends(get_db)):
...
def list_interventions(objective_id: int, db: Session = Depends(get_db)):
